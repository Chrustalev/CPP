...
TEST(VectorTest, Constructor_n_throw) {
  try {
    ...
    EXPECT_EQ(err.what(), std::string("cannot create s21::vector"));
  }
}
...
TEST(VectorTest, Element_at_throw) {
  ...
  EXPECT_ANY_THROW(static_cast<void>(std_v.at(1)));
}
...
TEST(VectorMoveTest, MoveConstructorCoverage) {
...
TEST(VectorMoveTest, SelfMoveAssignment) {
...
TEST(VectorInsertManyTest, InsertManyBack_BasicTypes) {
  s21::vector<int> v1;
  std::vector<int> v2;

  v1.insert_many_back(1, 2, 3);
...
TEST(VectorInsertManyTest, InsertManyBack_MultipleCalls) {
  s21::vector<int> v1;
  std::vector<int> v2;

  v1.insert_many_back(1);
  v1.insert_many_back(2, 3);
  v1.insert_many_back(4, 5, 6);

  v2.insert(v2.end(), {1});
  v2.insert(v2.end(), {2, 3});
  v2.insert(v2.end(), {4, 5, 6});
...
TEST(VectorInsertManyTest, InsertMany_AtBeginning) {
  s21::vector<int> v1{4, 5, 6};
  std::vector<int> v2{4, 5, 6};

  auto it1 = v1.insert_many(v1.begin(), 1, 2, 3);
  v2.insert(v2.begin(), {1, 2, 3});

  EXPECT_EQ(*it1, 1);  // Проверяем возвращаемый итератор
  EXPECT_EQ(v1.size(), v2.size());
  for (size_t i = 0; i < v1.size(); ++i) {
    EXPECT_EQ(v1[i], v2[i]);
  }
...
TEST(VectorInsertManyTest, InsertMany_InMiddle) {
  s21::vector<int> v1{1, 5, 6};
  std::vector<int> v2{1, 5, 6};

  auto it1 = v1.insert_many(v1.begin() + 1, 2, 3, 4);
  v2.insert(v2.begin() + 1, {2, 3, 4});
...
TEST(VectorInsertManyTest, InsertMany_AtEnd) {
  s21::vector<int> v1{1, 2, 3};
  std::vector<int> v2{1, 2, 3};

  auto it1 = v1.insert_many(v1.end(), 4, 5, 6);
  // Убираем неиспользуемую переменную
  v2.insert(v2.end(), {4, 5, 6});

  EXPECT_EQ(it1, v1.end() - 3);
  EXPECT_EQ(v1.size(), v2.size());
  for (size_t i = 0; i < v1.size(); ++i) {
    EXPECT_EQ(v1[i], v2[i]);
  }
...
// ================ С реаллокацией ================
TEST(VectorInsertManyTest, InsertMany_WithReallocation) {
  s21::vector<int> v1{1, 2, 3};
  std::vector<int> v2{1, 2, 3};

  // Принудительно делаем маленький capacity
  v1.reserve(3);
  v2.reserve(3);

  v1.insert_many(v1.begin() + 1, 10, 20, 30, 40, 50);
  v2.insert(v2.begin() + 1, {10, 20, 30, 40, 50});
...
// ================ Edge cases ================
TEST(VectorInsertManyTest, InsertMany_EmptyArgs) {
  s21::vector<int> v1{1, 2, 3};
  std::vector<int> v2{1, 2, 3};

  auto it1 = v1.insert_many(v1.begin() + 1);
  v2.insert(v2.begin() + 1, {});

  EXPECT_EQ(*it1, 2);  // Итератор указывает на элемент после позиции вставки
  EXPECT_EQ(v1.size(), v2.size());
  for (size_t i = 0; i < v1.size(); ++i) {
    EXPECT_EQ(v1[i], v2[i]);
  }
...
TEST(VectorInsertManyTest, InsertMany_Strings) {
  s21::vector<std::string> v1{"a", "d"};
  std::vector<std::string> v2{"a", "d"};

  v1.insert_many(v1.begin() + 1, "b", "c");
  v2.insert(v2.begin() + 1, {"b", "c"});
...
TEST(VectorInsertManyTest, InsertMany_MixedTypesWithConversion) {
  s21::vector<double> v1{1.5, 4.5};
  std::vector<double> v2{1.5, 4.5};

  v1.insert_many(v1.begin() + 1, 2, 3, 4);
  v2.insert(v2.begin() + 1, {2.0, 3.0, 4.0});
...
TEST(VectorInsertManyTest, InsertMany_ReturnValue) {
  s21::vector<int> v1{1, 2, 3, 4, 5};

  auto it = v1.insert_many(v1.begin() + 2, 10, 11, 12);
...
// ================ На пустом векторе ================
TEST(VectorInsertManyTest, InsertManyBack_OnEmptyVector) {
  s21::vector<int> v1;
  std::vector<int> v2;

  v1.insert_many_back(42);
  v2.push_back(42);

  EXPECT_EQ(v1.size(), v2.size());
  EXPECT_EQ(v1[0], v2[0]);

  v1.insert_many_back(43, 44);
  v2.insert(v2.end(), {43, 44});
...
TEST(VectorInsertManyTest, InsertMany_StressTest) {
  s21::vector<int> v1;
  std::vector<int> v2;

  const int count = 1000;
  for (int i = 0; i < count; i += 5) {
    v1.insert_many_back(i, i + 1, i + 2, i + 3, i + 4);
    v2.insert(v2.end(), {i, i + 1, i + 2, i + 3, i + 4});
  }
